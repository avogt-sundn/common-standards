 ...
 * <p>
 * Every violation maps to a rule in Common-Standards-Eclipse-Code-Profile.xml:
 * <ul>
 ...
 * </ul>
 */
public class FormatterShowcaseUnformatted2
{ // VIOLATION: { must be on next line (Allman style)
   ...
   */
  public FormatterShowcaseUnformatted2()
  { // VIOLATION: { must be on next line
  }
  ...
  { // VIOLATION: { must be on next line
    ...
    { // VIOLATION: missing space before (, { on same line
  ...
  { // VIOLATION: { must be on next line
    ...
    { // VIOLATION: missing space before (, spaces around operators
      throw new IllegalArgumentException("score must be in [0, 100], got: " + score);
    }
    else if (score < 25)
    { // VIOLATION: else must be on new line; missing space before (
      return Severity.LOW;
    }
    else if (score < 50)
    { // VIOLATION: else must be on new line
  ...
  { // VIOLATION: { must be on next line
    ...
    { // VIOLATION: missing space before (, { must be on next line
      case LOW -> "info";
      case MEDIUM -> "warning";
      case HIGH -> "error";
      case CRITICAL -> "fatal";
  ...
  { // VIOLATION: { must be on next line
    ...
    { // VIOLATION: missing space before (, { on same line
      ...
      { // VIOLATION: missing space before (
        results.add(transform.apply(event.message()));
      }
    }
    catch (IllegalArgumentException | NullPointerException e)
    { // VIOLATION: catch must be on new line; missing spaces around |
  ...
  { // VIOLATION: { must be on next line
    return events.stream().filter(filter).map(event -> { // VIOLATION: lambda { must be end-of-line, not on its own line
  ...
  { // VIOLATION: { must be on next line
    ...
    return "sum=" + sum + ", diff=" + difference + ", prod=" + product + ", relation=" + relation; // VIOLATION: missing
                                                                                                   // spaces around +
  }
